...
为了支持新的 `psi / Npl` 主线、样本级分层和 regime 诊断，当前代码运行时还会补算以下列；其中 `scripts/compute_feature_parameters.py` 在输入含 `Nexp (kN)` 时也会把 `psi` 一并持久化输出：
...
当前规则基于 `b/h` 和 `r_0/h` 的几何关系自动判定，用于 regime analysis 和后续分组实验。
